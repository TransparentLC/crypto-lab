...
const decodeEntities = (s: string) => s
    .replace(/&#(\d+);/g, (_, e: string) => String.fromCharCode(parseInt(e)))
    .replace(/&x([\dA-F]+);/gi, (_, e: string) => String.fromCharCode(parseInt(e, 16)))
    .replace(/&(.+?);/gi, (m, e: string) => ({
        amp: '&',
        apos: '\'',
        lt: '<',
        gt: '>',
...
const renderToken = (token: MarkedToken | EquationToken): string | number | boolean | VNode => {
    const r = (tokens: MarkedToken[]) => () => tokens.map(e => renderToken(e));
    switch (token.type) {
        case 'heading':
            return h([NH1, NH2, NH3, NH4, NH5, NH6][token.depth - 1], r(token.tokens as MarkedToken[]));
        case 'hr':
            return h(NDivider);
        case 'paragraph':
        ...
        case 'blockquote':
            ...
            if (token.tokens[0].type === 'paragraph' && (m = /\[@(.*?)\]/g.exec(token.tokens[0].raw))) {
                const summary = m[1].trim();
                return h('details', {}, [
                    // @ts-ignore
                    ...(summary ? [h('summary', (marked.lexer(summary) as MarkedToken[])[0].tokens.map(e => renderToken(e)))] : []),
                    ...r(token.tokens.slice(1) as MarkedToken[])(),
                ]);
            }
            return h(NBlockquote, r(token.tokens as MarkedToken[]));
        case 'list':
            return h(token.ordered ? NOl : NUl, r(token.items as MarkedToken[]));
        case 'list_item':
            return h(NLi, r(token.tokens as MarkedToken[]));
        case 'text':
            return ('tokens' in token) ? h(NText, r(token.tokens as MarkedToken[])) : decodeEntities(token.text);
        case 'link':
            return h(NA, {
                href: token.href,
        ...
        case 'table':
            return h(NTable, { style: { width: 'unset' }}, () => [
                h(NThead, () => [
                    h(NTr, () => token.header.map(
                        (e, i) => h(NTh, { style: { textAlign: token.align[i] }}, r(e.tokens as MarkedToken[]))
                    )),
                ]),
                h(NTbody, () => token.rows.map(
                    e => h(NTr, () => e.map(
                        (t, i) => h(NTd, { style: { textAlign: token.align[i] }}, r(t.tokens as MarkedToken[]))
                    ))
                )),
            ]);
        case 'equation':
...
export default (props => (marked.lexer(props.content) as MarkedToken[]).map(e => renderToken(e))) satisfies FunctionalComponent<{ content: string }>;
