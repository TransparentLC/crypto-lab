...
const dir: typeof console.dir = (obj, options) =>
    process.env.NODE_ENV === 'production' || console.dir(obj, options);

const log: typeof console.log = (...data) =>
    process.env.NODE_ENV === 'production' || console.log(...data);

export const judge = async (
    submission: InferSelectModel<typeof submissions>,
): Promise<Partial<InferSelectModel<typeof submissions>>> => {
    try {
        ...
        for (const checkpoint of checkpointMetadata) {
            const checkpointInput = await checkpointZip.stream(
                checkpoint.input,
            );
...
export const judgeLoop = async () => {
    while (true) {
        const submission = db
            .select()
            .from(submissions)
            .where(and(not(submissions.obsolete), submissions.pending))
        ...
        eventEmitter.emit('judge', {
            ...
            uid: submission.uid,
            title,
            // biome-ignore-start lint/style/noNonNullAssertion: 这几项必定存在
            compileSuccess: result.compileSuccess!,
            time: result.time!,
            memory: result.memory!,
            accepted: result.accepted!,
            acceptedCount: result.acceptedCount!,
            // biome-ignore-end lint/style/noNonNullAssertion: 这几项必定存在
        });
        if (result.accepted) {
            const row = db
                .select({
