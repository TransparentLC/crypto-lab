...
export const experiments = sqliteTable('experiments', {
    ...
    reportSubmission: integer('report_submission', { mode: 'boolean' }).default(true).notNull(),
    // 1000 -> 1 vCPU
    ...
    compileCommands: text('compile_commands', { mode: 'json' }).$type<Record<string, string>>().notNull(),
    // 一个zip压缩包
    // check0.in.txt
    // check0.out.txt
    // check1.in.bin
...
export const reports = sqliteTable('reports', {
    ...
    expid: integer('expid').notNull().references(() => experiments.expid),
...
export const submissions = sqliteTable('submissions', {
    ...
    expid: integer('expid').notNull().references(() => experiments.expid),
    // 提交时间 ISO 8601
    submitTime: text('submit_time').$defaultFn(() => (new Date).toISOString()).notNull(),
